      ...
      <div className="grid gap-5">
            ...
            {THEME_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative rounded-xl p-4 flex flex-col items-center gap-2.5 transition-all duration-200 text-center ring-1",
                    active
        ...
        <SettingsSection title="Quiet Hours" icon={<Clock className="h-4 w-4" />} delay={0.1}>
          <div className="space-y-3">
            ...
            {quietHoursEnabled && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-center gap-3 pl-10">
                <input type="time" value={quietStart} onChange={(e) => setQuietStart(e.target.value)} className="rounded-lg border border-sentinel-border bg-transparent text-foreground text-sm px-3 py-2 focus:ring-1 focus:ring-sentinel-accent-cyan/40 focus:outline-none" />
                <span className="text-xs text-muted-foreground">to</span>
                <input type="time" value={quietEnd} onChange={(e) => setQuietEnd(e.target.value)} className="rounded-lg border border-sentinel-border bg-transparent text-foreground text-sm px-3 py-2 focus:ring-1 focus:ring-sentinel-accent-cyan/40 focus:outline-none" />
              </motion.div>
            )}
          </div>
        </SettingsSection>
...
function SettingsRow({ label, desc, icon, children }: { label: string; desc: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 py-3 border-b border-sentinel-border/30 last:border-0">
