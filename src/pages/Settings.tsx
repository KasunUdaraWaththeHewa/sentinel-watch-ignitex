...
const Settings = () => {
  ...
  return (
    <AppLayout
      title="Settings"
      subtitle="Configure how Sentinel works for you."
    >
      <div className="grid gap-5">
        ...
        >
          <div className="space-y-1">
            <SettingsRow
              ...
              desc="Get notified via email before deadlines"
              icon={<Mail className="h-4 w-4 text-sentinel-accent-cyan" />}
            >
              <Switch
            ...
            <SettingsRow
              ...
              icon={<Bell className="h-4 w-4 text-sentinel-accent-cyan" />}
            >
              <Switch
            ...
            <SettingsRow
              label="Weekly Sentinel Briefing"
              desc="Summary of upcoming items every Monday"
              icon={<Mail className="h-4 w-4 text-sentinel-accent-cyan" />}
            >
              <Switch
        ...
        >
          <div className="space-y-3">
            <SettingsRow
              ...
              desc="No notifications during this window"
              icon={<Clock className="h-4 w-4 text-sentinel-accent-cyan" />}
            >
              <Switch
            ...
            {quietHoursEnabled && (
              <motion.div
                ...
                className="grid gap-2 pl-0 sm:grid-cols-[auto_auto_auto] sm:items-center sm:pl-10"
              >
                <input
                  type="time"
                  value={quietStart}
                  onChange={(e) => setQuietStart(e.target.value)}
                  className="w-full rounded-lg border border-sentinel-border bg-transparent text-foreground text-sm px-3 py-2 focus:ring-1 focus:ring-sentinel-accent-cyan/40 focus:outline-none"
                />
        ...
        >
          <div className="space-y-1">
            <SettingsRow
              label="Escalate overdue critical items"
              desc="Increase urgency when critical renewals become overdue"
              icon={<Zap className="h-4 w-4 text-sentinel-accent-cyan" />}
            >
              <Switch
            ...
            <SettingsRow
              label="Mobile-friendly summary card"
              desc="Prioritize compact cards for quick scans on mobile"
              icon={
                <Smartphone className="h-4 w-4 text-sentinel-accent-cyan" />
              }
            >
              <Switch
                checked={mobileSummary}
                onCheckedChange={setMobileSummary}
              />
            </SettingsRow>
            <div className="grid gap-2 rounded-xl bg-muted/25 px-3 py-3 sm:grid-cols-[minmax(0,1fr)_180px] sm:items-center sm:gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Trigger alerts before high-risk deadlines
                </p>
              </div>
              <select
              ...
              >
                ...
                <option value="24" className="bg-background text-foreground">
                  24 hours
                </option>
                <option value="48" className="bg-background text-foreground">
                  48 hours
                </option>
                <option value="72" className="bg-background text-foreground">
                  72 hours
                </option>
              </select>
            </div>
        ...
        >
          <select
            ...
            className="w-full rounded-xl border border-sentinel-border bg-transparent text-foreground text-sm px-3 py-3 appearance-none cursor-pointer focus:ring-1 focus:ring-sentinel-accent-cyan/40 focus:outline-none"
          >
            {TIMEZONES.map((tz) => (
              <option
                key={tz}
        ...
        >
          <div className="space-y-1">
            <SettingsRow
              ...
              desc="Receive a monthly account and activity digest"
              icon={
                <ShieldCheck className="h-4 w-4 text-sentinel-accent-cyan" />
              }
            >
              <Switch
                checked={securityDigest}
                onCheckedChange={setSecurityDigest}
              />
            </SettingsRow>
            <div className="grid gap-2 rounded-xl bg-muted/25 px-3 py-3 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-center sm:gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Choose who can view risk scores in your workspace
                </p>
              </div>
              <select
              ...
              >
                ...
                <option value="team" className="bg-background text-foreground">
                  My team
                </option>
