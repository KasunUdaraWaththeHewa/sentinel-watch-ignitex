...
const ItemDetail = () => {
      ...
      >
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            This watched item may have been removed or doesn't exist.
          </p>
          <Button
  ...
  if (item.schedule !== Schedule.OneTime) {
    ...
    for (let i = 0; i < 3; i++) {
      nextOccurrences.push(
        new Date(item.nextDate.getTime() + i * intervalDays * MS_PER_DAY),
      );
    }
  } else {
  ...
  const timeline = [
    ...
    ...(daysLeft <= 14
      ? [
          {
            date: new Date(item.nextDate.getTime() - 7 * MS_PER_DAY),
            label: "Reminder sent",
            icon: Bell,
            active: true,
          },
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          "glass-surface rounded-2xl p-6 sm:p-8 space-y-5",
          isUrgent && "ring-1 ring-sentinel-severity-high/15",
        )}
      >
        <div className="flex items-start gap-4">
          <div
            className={cn(
              ...
              isUrgent
                ? "bg-sentinel-severity-high/10"
                : "bg-sentinel-accent-cyan/10",
            )}
          >
            <Icon
              className={cn(
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <InfoBlock
            ...
            value={timeRemaining}
            highlight={isUrgent}
            accent={isUrgent ? "high" : "cyan"}
          />
          <InfoBlock
            icon={<Calendar className="h-4 w-4" />}
            label="Due Date"
            value={formatFullDate(item.nextDate)}
            accent="cyan"
          />
          <InfoBlock
            icon={<Shield className="h-4 w-4" />}
            label="Regret Risk"
            value={item.regretRisk}
            highlight={item.regretRisk === RegretRisk.High}
            accent={item.regretRisk === RegretRisk.High ? "high" : "cyan"}
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          {item.status !== ItemStatus.Handled && (
            ...
            >
              <CheckCircle2 className="h-4 w-4" /> Mark as Handled
            </Button>
          )}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-sm text-foreground font-medium">
            {SCHEDULE_LABELS[item.schedule] || item.schedule}
          </p>
          <div className="space-y-0">
            ...
            {nextOccurrences.map((d, i) => (
              <div
                key={i}
                className="flex items-center gap-3 py-2.5 border-b border-sentinel-border/50 last:border-0"
              >
                <div
                  className={cn(
                    "w-2 h-2 rounded-full flex-shrink-0",
                    i === 0 ? "bg-sentinel-accent-cyan" : "bg-muted",
                  )}
                />
                ...
                {i === 0 && (
                  <span className="text-[10px] text-sentinel-accent-cyan font-medium ml-auto uppercase tracking-wider">
                    Next
                  </span>
                )}
              </div>
      ...
      {item.notes && (
        <motion.div
          ...
          className="glass-surface rounded-2xl p-6 space-y-3"
        >
          <SectionHeader
...
function InfoBlock({
  ...
  return (
    <div
      className={cn(
        "rounded-xl p-3.5 space-y-1.5 ring-1",
        accent === "high" && highlight
          ? "bg-sentinel-severity-high/5 ring-sentinel-severity-high/10"
    ...
    >
      ...
      <p
        className={cn(
          ...
          highlight ? "text-sentinel-severity-high" : "text-foreground",
        )}
      >
        {value}
      </p>
