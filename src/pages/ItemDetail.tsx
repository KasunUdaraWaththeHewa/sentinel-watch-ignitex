...
import { ArrowLeft, Clock, Calendar, Shield, Repeat, Bell, CheckCircle2, Pause, FileText, ExternalLink } from "lucide-react";
...
const ItemDetail = () => {
    ...
    return (
      <div className="min-h-screen bg-background bg-noise">
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-16 text-center space-y-4">
          <p className="text-foreground font-display text-xl font-semibold">Item not found</p>
          <Button variant="glass" onClick={() => navigate("/")}>Back to Command Center</Button>
        </main>
        <BottomNav />
      </div>
    );
  ...
  if (item.schedule !== "one-time") {
    const intervalDays =
      item.schedule === "daily" ? 1 :
      item.schedule === "monthly" ? 30 :
      item.schedule === "quarterly" ? 90 :
      item.schedule === "yearly" ? 365 : 30;
    for (let i = 0; i < 3; i++) {
  ...
  return (
    <div className="min-h-screen bg-background bg-noise">
      <TopNav />
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 pb-28 md:pb-12 space-y-6">
        {/* Back */}
        <button
          onClick={() => navigate(-1)}
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            />
            <InfoBlock
              icon={<Calendar className="h-4 w-4 text-sentinel-accent-cyan" />}
              label="Due Date"
              value={formatFullDate(item.nextDate)}
            />
            <InfoBlock
              icon={<Shield className="h-4 w-4 text-sentinel-accent-cyan" />}
              label="Regret Risk"
              value={item.regretRisk}
              highlight={item.regretRisk === "High"}
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button variant="glass-primary" className="flex-1 gap-2">
            ...
            {nextOccurrences.map((d, i) => (
              <div key={i} className="flex items-center gap-3 py-2 border-b border-sentinel-border last:border-0">
                <div className={cn(
                  ...
                  i === 0 ? "bg-sentinel-accent-cyan" : "bg-sentinel-surface"
                )} />
                ...
                <div key={i} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={cn(
                      ...
                      event.active ? "bg-sentinel-accent-cyan/15" : "bg-sentinel-surface"
                    )}>
